// Vendored from mind-ar@1.2.5 (npm pack, no `npm install` — see README's
// AR section for why). PATCHED by hand: the original bundle imported the
// removed three.js export `sRGBEncoding` and assigned it to the equally
// removed `WebGLRenderer.outputEncoding` (three.js dropped both in favor
// of `outputColorSpace`/`SRGBColorSpace` well before three@0.185, this
// project's pinned version) — that's a hard rollup MISSING_EXPORT build
// error, not a lint nitpick. Fixed by dropping the dead import and
// assigning `renderer.outputColorSpace = "srgb"` directly, three's own
// default anyway. No other behavior changed.
